...
namespace TomatoTool
{
	public class MapArea : ROMObject
	{
		...
		{
			address = tomatoAdventure.readAsAddress(tomatoAdventure.readAsAddress(TopAddressPointer[(int)SelectTopAddressArrayDoublePointer]) + (map.Number * TomatoTool.ROM.ADDRESS_SIZE));
		...
		{
			this.address = tomatoAdventure.malloc(getSize());
			tomatoAdventure.writeAsAddress(tomatoAdventure.readAsAddress(TopAddressPointer[(int)SelectTopAddressArrayDoublePointer]) + (map.Number * TomatoTool.ROM.ADDRESS_SIZE), address);
		...
		public void draw(Graphics graphics, bool transparent)
		{
			if (area != null)
			{
				...
				cm.Matrix11 = 1;
				cm.Matrix22 = 1;
				cm.Matrix33 = transparent ? (float)0.5 : 1;
				cm.Matrix44 = 1;
