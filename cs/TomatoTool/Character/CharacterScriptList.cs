...
namespace TomatoTool
{
	public class CharacterScriptList : ROMObject
	{
		...
		public static readonly uint PALETTE_LENGTH_0 = 12;
		public static readonly uint CHARACTER_IMAGE_LENGTH_0 = 16;
		...
		public override void load(TomatoAdventure tomatoAdventure, uint address)
		{
			...
			{
				if (tomatoAdventure.isAddress(tomatoAdventure.readAsAddress(address + (i * ROM.ADDRESS_SIZE))))
				{
					characterImage[i] = (CharacterImage)tomatoAdventure.add(new CharacterImage(tomatoAdventure, tomatoAdventure.readAsAddress(address + (i * ROM.ADDRESS_SIZE))));
				}
				else
				{
			...
			{
				if (tomatoAdventure.isAddress(tomatoAdventure.readAsAddress(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (i * ROM.ADDRESS_SIZE))))
				{
					palette[i] = (Palette)tomatoAdventure.add(new Palette(tomatoAdventure, tomatoAdventure.readAsAddress(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (i * ROM.ADDRESS_SIZE))));
				}
			}

			{
				...
				uint i = 0;
				while (tomatoAdventure.read(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (i * TomatoTool.CharacterScript.SIZE)) != 0xFF)
				{
					characterScript.Add(new CharacterScript(tomatoAdventure, address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (i * TomatoTool.CharacterScript.SIZE)));
					++i;
				}
			}
		}
		...
		public override void save(TomatoAdventure tomatoAdventure, uint address)
		{
			...
			for (uint i = 0; i < characterImage.GetLength(0); ++i)
			{
				if ((characterImage[i] != null) && characterImage[i] != TomatoTool.CharacterImage.NULL)
				{
					...
					tomatoAdventure.writeAsAddress(address + (i * ROM.ADDRESS_SIZE), characterImage[i].Address);
				}
				else
				{
			...
			for (uint i = 0; i < palette.GetLength(0); ++i)
			{
				if (palette[i] != null)
				{
					...
					tomatoAdventure.writeAsAddress(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (i * ROM.ADDRESS_SIZE), palette[i].Address);
				}
				else
				{
					tomatoAdventure.writeAsAddress(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (i * ROM.ADDRESS_SIZE), palette[0].Address);
				}
			}
			...
			for (uint i = 0; i < characterScript.Count; ++i)
			{
				...
				{
					characterScript[(int)i].save(tomatoAdventure, address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (i * TomatoTool.CharacterScript.SIZE));
				}
			}

			//フッター
			tomatoAdventure.write((uint)(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (characterScript.Count * TomatoTool.CharacterScript.SIZE) + 0), 0xFF);
			tomatoAdventure.write((uint)(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (characterScript.Count * TomatoTool.CharacterScript.SIZE) + 1), 0x00);
			tomatoAdventure.write((uint)(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (characterScript.Count * TomatoTool.CharacterScript.SIZE) + 2), 0x00);
			tomatoAdventure.write((uint)(address + (CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (characterScript.Count * TomatoTool.CharacterScript.SIZE) + 3), 0x00);
		}

		public override uint getSize()
		{
			return (uint)((CHARACTER_IMAGE_LENGTH_0 * ROM.ADDRESS_SIZE) + (PALETTE_LENGTH_0 * ROM.ADDRESS_SIZE) + (characterScript.Count * TomatoTool.CharacterScript.SIZE) + FOOTER_SIZE);
		}
