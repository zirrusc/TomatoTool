...
namespace TomatoTool
{
	public partial class FormMapEditor : Form
	{
		...
		{
			if (map.MapGraphic.ChipSetList[radioButtonChipSetListBG()] != null)
			{
				listBoxChipSetList.DataSource = map.MapGraphic.ChipSetList[radioButtonChipSetListBG()].ChipSet;
		...
		{
			//塗りつぶし透過させないため
			e.Graphics.FillRectangle(new SolidBrush(map.MapGraphic.Palette[0][0].toColor()), 0, 0, ((PictureBox)sender).Width, ((PictureBox)sender).Height);

			//拡大描写の描写方法指定
			e.Graphics.InterpolationMode = InterpolationMode.NearestNeighbor;
			e.Graphics.PixelOffsetMode = PixelOffsetMode.Half;

			e.Graphics.DrawImage(map.MapGraphic.MapTile.MainTile.toBitmap(selectChip, map.MapGraphic.Palette), 0, 0, (Chip.WIDTH * 4), (Chip.HEIGHT * 4));
		}
		...
		private void pictureBoxChipSetListCopyList_Paint(object sender, PaintEventArgs e)
		{
			...
			((PictureBox)sender).Height = (map.MapGraphic.MapTile.MainTile.Count / (Chip.HEIGHT * 2)) * 16;
			...
			//タイル描写
			for (int i = 0; i < map.MapGraphic.MapTile.MainTile.Count; ++i)
			{
				using (Bitmap bitmap = map.MapGraphic.MapTile.MainTile[i].toBitmap(map.MapGraphic.Palette[trackBarChipSetListCopyListPalette.Value], chip.FlipX, chip.FlipY))
				{
		...
		private void pictureBoxChipSetListCopyList_MouseMove(object sender, MouseEventArgs e)
		{
			if ((e.Button == MouseButtons.Left) &&
				...
				((e.X / (Chip.WIDTH * 2)) + ((e.Y / (Chip.HEIGHT * 2)) * 16) < map.MapGraphic.MapTile.MainTile.Count))
			{
		...
		private void pictureBoxChipSetList_Paint(object sender, PaintEventArgs e)
		{
			...
			if (chipSetList != null)
			{
				...
				e.Graphics.FillRectangle(new SolidBrush(mapGraphic.Palette[0][0].toColor()), 0, 0, ((PictureBox)sender).Width, ((PictureBox)sender).Height - (ChipSet.HEIGHT * 2));
				e.Graphics.FillRectangle(new SolidBrush(mapGraphic.Palette[0][0].toColor()), 0, ((PictureBox)sender).Height - (ChipSet.HEIGHT * 2), (chipSetList.Count % 16 == 0 ? 16 : chipSetList.Count % 16) * (ChipSet.WIDTH * 2), ((PictureBox)sender).Height);
