...
namespace TomatoTool
{
	public partial class FormMapEditor : Form
	{
		private void comboBoxMapScriptList_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			comboBoxMapScriptListMainScript.DataSource = tomatoAdventure.ObjectList[typeof(Script)];
			comboBoxMapScriptListMainScript.SelectedIndexChanged += comboBoxMapScriptListMainScript_SelectedIndexChanged;
		...
		{
			e.Value = String.Format("{0:X8}", ((MapScriptList)e.ListItem).Address);
		}
		...
		private void componentMapScriptLoad(MapScript mapScript)
		{
			if (mapScript != null)
			{
				...
				textBoxMapScriptScriptAddress.Text = String.Format("{0:X8}", mapScript.Script.Address);
		...
		{
			e.Value = String.Format("{0:X8}", ((Script)e.ListItem).Address);
		}
		...
		private void textBoxMapScriptScriptAddress_TextChanged(object sender, EventArgs e)
		{
			if ((listBoxMapScript.SelectedItem != null) && (((TextBox)sender).Text != null) && (((TextBox)sender).Text != ""))
			{
				try
				{
					((MapScript)listBoxMapScript.SelectedItem).Script.Address = Convert.ToUInt32(((TextBox)sender).Text, 16);
				}
				catch
				{
					((TextBox)sender).Text = String.Format("{0:X8}", ((MapScript)listBoxMapScript.SelectedItem).Script.Address);
				}
			}
		}
