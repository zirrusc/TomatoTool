...
namespace TomatoTool
{
	public partial class FormMapEditor
	{
		...
		private void componentCharacterScriptLoad(CharacterScript characterScript)
		{
			...
			textBoxCharacterScriptActionScriptAddress.Text = String.Format("{0:X8}", characterScript.ActionScript.Address);
			textBoxCharacterScriptScriptAddress.Text = String.Format("{0:X8}", characterScript.Script.Address);
		...
		{
			e.Value = String.Format("{0:X8}", ((CharacterImage)e.ListItem).Address);
		}
		...
		private void textBoxCharacterScriptActionScriptAddress_Validating(object sender, CancelEventArgs e)
		{
			if (listBoxCharacterScript.SelectedItem != null)
			{
				...
				try
				{
					characterScript.ActionScript.Address = Convert.ToUInt32(textBox.Text, 16);
					pictureBoxCharacterScript.Refresh();
				}
				catch
				{
					switch (MessageBox.Show("エラー", FormTomatoTool.applicationName, MessageBoxButtons.OKCancel, MessageBoxIcon.Error))
					{
						case DialogResult.Cancel:
							...
							textBox.Text = String.Format("{0:X8}", characterScript.ActionScript.Address);
		...
		private void textBoxCharacterScriptScriptAddress_Validating(object sender, CancelEventArgs e)
		{
			if (listBoxCharacterScript.SelectedItem != null)
			{
				...
				try
				{
					characterScript.Script.Address = Convert.ToUInt32(textBox.Text, 16);
					pictureBoxCharacterScript.Refresh();
				}
				catch
				{
					switch (MessageBox.Show("エラー", FormTomatoTool.applicationName, MessageBoxButtons.OKCancel, MessageBoxIcon.Error))
					{
						case DialogResult.Cancel:
							...
							textBox.Text = String.Format("{0:X8}", characterScript.Script.Address);
		...
		private void buttonCharacterScriptActionScriptAddressEdit_Click(object sender, EventArgs e)
		{
			if (listBoxCharacterScript.SelectedItem != null)
			{
				...
				ScriptEditor(tomatoAdventure, characterScript.ActionScript.Address);
			}
		}
		private void buttonCharacterScriptScriptEdit_Click(object sender, EventArgs e)
		{
			if (listBoxCharacterScript.SelectedItem != null)
			{
				...
				ScriptEditor(tomatoAdventure, characterScript.Script.Address);
			}
		}
